let currentPage = 1;
const formData = {
    id: null, // ID único para el registro
    datosPersonales: {},
    familiares: [],
    condicionesSalud: [],
    internamientos: []
...
if (localStorage.getItem('registroFormulario')) {
    const savedData = JSON.parse(localStorage.getItem('registroFormulario'));
...
function saveData(page) {
    if (page === 1) {
        ...
        if (!formData.id) formData.id = Date.now(); // Asigna un ID único si no existe
    } else if (page === 2) {
...
function showSummary() {
    ...
    resumen.innerHTML = `
        <h3>Datos Personales (ID: ${formData.id})</h3>
        ...
        ${formData.condicionesSalud.length > 0 ? formData.condicionesSalud.map(c => `<p>${c.enfermedad}, ${c.tiempo}</p>`).join('') : '<p>No hay condiciones registradas.</p>'}
...
document.getElementById('multiPageForm').addEventListener('submit', function(e) {
    ...
    const jsonData = JSON.stringify(formData); // Convertir a JSON
    localStorage.setItem('registroFormulario', jsonData); // Guardar en localStorage
    downloadJSON(formData, `registro_${formData.id}.json`); // Descargar como archivo
    console.log(jsonData); // Mostrar en consola
    alert('Formulario enviado, guardado en localStorage y descargado como archivo JSON!');
});